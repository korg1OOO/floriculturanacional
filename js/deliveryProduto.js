$(document).ready(function () {
	...
	$(document).on('keyup', 'input[name="pesquisar"]', function (e) {
		...
		setTimeout(() => {
			if (registros == 0) {
				$('.form-busca span').html('Ṇo encontramos resultados para sua busca!');
			} else {
...
function totalProduto() {
    ...
    $("#add-ons .opcoes").each(function () {
        let preco = parseFloat($(this).find(".preco span[style='display:none;']").text()) || 0;
        let qtde = parseInt($(this).find("input.qtdeOpcao").val()) || 0;
        console.log('Add-on:', $(this).find(".nome b").text(), 'Price:', preco, 'Quantity:', qtde);
