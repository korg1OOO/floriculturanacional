$(document).ready(function () {
	...
	$(document).on('click', 'button#btFinalizar', function (e) {
		...
		verificarConexaoInternet().then(status => {
			if (status === 'conectado') {
				...
				mostrarPopup('Sem ConexÃƒÂ£o com a Internet',
					'Parece que vocÃƒÂª estÃƒÂ¡ offline. Verifique sua conexÃƒÂ£o com a internet e tente novamente.');
				return;
			}
		});
	});
	...
	$(document).on('click', '.adicionarProduto', function (e) {
		...
		if (clicks === 1) {
				...
				verificarConexaoInternet().then(status => {
					if (status === 'conectado') {
						...
						if (lojaAberta.trim() == 's') {
							...
							$('#modalCarregando').hide();
							$("#modal button").addClass('confirmar');
							mostrarPopup('Esta loja estÃƒÂ¡ fechada no momento.',
								'Mas vocÃƒÂª pode olhar Ãƒ  vontade e voltar quando a loja estiver aberta.');
							return
						}
					} else {
						$('#modalCarregando').hide();
			...
			clicks = 0;             //apos fazer a aÃƒÂ§ÃƒÂ£o reseta o timer
		}
	...
	$(document).on('click', '#btEnviarPedido', function (e) {
		...
		verificarConexaoInternet().then(status => {
			if (status === 'conectado') {
				...
				if ($('#nome').val().trim() == '') {
					mostrarPopup('Nome obrigatÃƒÂ³rio',
				...
				if ($('#telefone').val().trim() == '' || $('#telefone').val().length < 14) {
					mostrarPopup('Telefone obrigatÃƒÂ³rio',
						'Por favor, digite o nÃƒÂºmero do seu celular.');
					$('#telefone').focus();
					$('#modalCarregando').hide();
					return
				}
				...
				if (entrega == undefined) {
					mostrarPopup('Entrega obrigatÃƒÂ³rio',
				...
				if (entrega.toLowerCase() === 'entrega') {
					if ($('#bairro').val().trim() == -1) {
						mostrarPopup('Bairro obrigatÃƒÂ³rio',
					...
					if ($('#endereco').val().trim() == '') {
						mostrarPopup('EndereÃƒÂ§o obrigatÃƒÂ³rio',
							'Por favor, informe o endereÃƒÂ§o para entrega.');
						$('#endereco').focus();
						$('#modalCarregando').hide();
						return
					}
					if ($('#numero').val().trim() == '') {
						mostrarPopup('NÃƒÂºmero obrigatÃƒÂ³rio',
							'Por favor, informe o nÃƒÂºmero do local para entrega.');
						$('#numero').focus();
						$('#modalCarregando').hide();
						return
					}
				...
				if (pagamento == undefined) {
					mostrarPopup('Forma de pagamento obrigatÃƒÂ³rio',
				...
				$.ajax({
					...
					success: function (data) {
						...
						if (obj.semEstoque) {
							...
							let msg = "Desculpe, mas tem produtos do seu carrinho que nÃƒÂ£o temos mais em estoque.";
							...
							if (obj.rastreamento) {
								if (obj.pedido) {
									...
									let urlWhatsapp = "https://wa.me/55" + obj.telefone + "?text=Oi, meu nome ÃƒÂ© *" + nome + "* e acabei de fazer o pedido de nÃƒÂºmero *" + obj.pedido + "* no seu cardÃƒÂ¡pio digital.%20%0A%0A%C2%B0%C2%B0%C2%B0%20*Acompanhe seu pedido aqui*%20%C2%B0%C2%B0%C2%B0%0A%0A " + (obj.rastreamento);

									/*window.open(
										"https://wa.me/55" + obj.telefone + "?text=Oi, meu nome ÃƒÂ© *" + nome + "* e acabei de fazer o pedido de nÃƒÂºmero *" + obj.pedido + "* no seu cardÃƒÂ¡pio digital.%20%0A%0A%C2%B0%C2%B0%C2%B0%20*Acompanhe seu pedido aqui*%20%C2%B0%C2%B0%C2%B0%0A%0A " + (obj.rastreamento).replace('https://', ''),
										"_blank" // <- This is what makes it open in a new window.
									);*/
									$('#pedido .containerFinalizar form').hide('slow');
									$('#pedido .containerFinalizar #obrigado h2').html("Pedido " + obj.pedido + " concluÃƒÂ­do com sucesso!");
									$('#pedido .containerFinalizar #obrigado h3').html("Favor encaminhar seu pedido via WhatsApp para agilizar o seuÃ‚ atendimento.");
								...
								if (obj.telefone) {
									...
									meuPedido = meuPedido.replaceAll('Taxa de cartÃƒÂ£o:', '*Taxa de cartÃƒÂ£o:*');
									meuPedido = meuPedido.replace('Cliente:', '*Cliente:*');
									meuPedido = meuPedido.replaceAll('&', '%26');
									meuPedido = meuPedido.replaceAll('#', '%23');

									let urlWhatsapp = "https://wa.me/55" + obj.telefone + "?text=Oi, meu nome ÃƒÂ© *" + nome + "* e acabei de fazer o pedido no seu cardÃƒÂ¡pio digital, segue abaixo:%0A%0A" + meuPedido;
	...
	$(document).on('click', '.loginEmail', function (e) {
		...
		if ($('#email').val().trim() == '' || $('#email').val().indexOf("@") == -1) {
			mostrarPopup('E-mail obrigatÃƒÂ³rio',
		...
		setTimeout(function () {
			...
			$.ajax({
				...
				error: function () {
					$('.carregando').hide('slow');
					let msg = "Desculpe, houve um erro ao enviar o cÃƒÂ³digo para o seu e-mail. Tente novamente mais tarde.";
					mostrarPopup('Ops! ', msg)
				}
			});
		}, 2000);
	...
	$(document).on('click', '.confirmarCodigo', function (e) {
		...
		if ($('#codigoConfirmacao').val().trim() == '' || $('#codigoConfirmacao').val().length < 11) {
			mostrarPopup('CÃƒÂ³digo obrigatÃƒÂ³rio',
				'Por favor, preencha o campo com o cÃƒÂ³digo.');
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (data) {
						...
						let msg = "Desculpe, mas o cÃƒÂ³digo informado nÃƒÂ£o estÃƒÂ¡ correto.";
						mostrarPopup('Ops! ', msg);
					}
					return false
				},
				error: function (data) {
					$('.carregando').hide('slow');
					//console.log(data);
					let msg = "Desculpe, houve um erro com a verificaÃƒÂ§ÃƒÂ£o do seu cÃƒÂ³digo. Tente novamente mais tarde.";
					mostrarPopup('Ops! ', msg);
				}
			});
		}, 2000);
...
function atualizarResumo() {
	...
	let pagamento = $("input[name='pagamento']:checked").val();
	if (pagamento != undefined) {
		if (item.val().toLowerCase().indexOf('cartÃƒÂ£o de crÃƒÂ©dito#') !== -1 || item.val().toLowerCase().indexOf('cartÃƒÂ£o de dÃƒÂ©bito#') !== -1) {
	...
	if (total <= 0) {
		$(".pagamentos").hide();
		$(".pagamentos").after('<div class="alert alert-success">ParabÃƒÂ©ns! Com a troca de pontos esse pedido vai sair totalmente GRATUITO! :)</div>');
	} else {
		$(".alert-success").remove();
		$(".pagamentos").show();
	}
...
function removerAcentuacao(string) {
	const comAcentos = ['Ãƒ ', 'ÃƒÂ¡', 'ÃƒÂ¢', 'ÃƒÂ£', 'ÃƒÂ¤', 'ÃƒÂ¥', 'ÃƒÂ§', 'ÃƒÂ¨', 'ÃƒÂ©', 'ÃƒÂª', 'ÃƒÂ«', 'ÃƒÂ¬', 'ÃƒÂ­', 'ÃƒÂ®', 'ÃƒÂ¯', 'ÃƒÂ±', 'ÃƒÂ²', 'ÃƒÂ³', 'ÃƒÂ´', 'ÃƒÂµ', 'ÃƒÂ¶', 'ÃƒÂ¹', 'ÃƒÂ¼', 'ÃƒÂº', 'ÃƒÂ¿', 'Ãƒâ‚¬', 'ÃƒÂ', 'Ãƒâ€š', 'ÃƒÆ’', 'Ãƒâ€ž', 'Ãƒâ€¦', 'Ãƒâ€¡', 'ÃƒË†', 'Ãƒâ€°', 'ÃƒÅ ', 'Ãƒâ€¹', 'ÃƒÅ’', 'ÃƒÂ', 'ÃƒÅ½', 'ÃƒÂ', 'Ãƒâ€˜', 'Ãƒâ€™', 'Ãƒâ€œ', 'Ãƒâ€', 'Ãƒâ€¢', 'Ãƒâ€“', 'O', 'Ãƒâ„¢', 'ÃƒÅ“', 'ÃƒÅ¡'];
...
function copiarChavePix(item) {
	...
	setTimeout(() => {
		item.find('.copiado').html('Copiado para ÃƒÂrea de transferÃƒÂªncia').show(0).delay(2000).hide(0);
	}, 2000);
	return false
}
